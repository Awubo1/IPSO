...
class   PSO():
    def __init__(self,jizhan, mes,pop, max_iter):  #   基站个数  边缘服务器个数，群体数量，迭代次数
        self.mes = mes  # 服务器个数
        self.jizhan = jizhan# #基站个数
    ...
    def read(self):
        xlsx1 = xlrd.open_workbook(r"D:\边缘计算\ipso\code\jizhan.xlsx")
        bs_ = xlsx1.sheets()[0]
        ...
        xlsx2 = xlrd.open_workbook(r"D:\边缘计算\ipso\code\fuwuqi.xlsx")
        ...
        xlsx3 = xlrd.open_workbook(r"D:\边缘计算\ipso\code\2500task200-1000.xlsx")
        ...
        xlsx4 = xlrd.open_workbook(r"D:\边缘计算\ipso\code\e.xlsx")
    ...
    def init_Population(self):  # 初始化种群
        self.population = np.zeros((self.pop, self.jizhan,self.mes),dtype= np.int)
        self.V=np.zeros((self.pop, self.jizhan,self.mes),dtype= np.int)
        self.choice1 = [i for i in range(self.mes + 1)]
        choice2 = [-1, 0, 1]
        for i in range(self.pop):
            # for j in range(self.jizhan):
            j=0
            while j<self.jizhan:
                for k in range(self.mes):
                    self.population[i][j][k] = random.choice(self.choice1)
                    self.V[i][j][k] = random.choice(choice2)# 初始化各个粒子的速度
    ...
    def com_fit1(self):
        ...
        self.iter_bfit.append(min_fitness)
        ##更新种群历史最优
    ...
    def com_parallel(self, gene):
        ...
        self.init_task_distance_infor = []
        for i in range(self.bs_num):
            AP_corresponding = gene[i]
            task_part = 0
            nonull_index = []
            for z in range(self.mes):
        ...
        return [f1, f2]
    def velocity_gene_update(self,V, X, pbest, gbest):
        w = 0.5
        r1 = np.random.random((self.jizhan, self.mes))
        r2 = np.random.random((self.jizhan, self.mes))
        for i in range(self.jizhan):
            j=0
            is_all_0=0
            while j <self.mes:
                vi=w*V[i][j]+self.c1 * r1[i][j] *(pbest[i][j]-X[i][j])+self.c2 * r2[i][j] * (gbest[i][j] - X[i][j])
                vi=int(vi)
                if vi < -self.max_val:
                    vi= -self.max_val
                ...
                if is_all_0==self.mes:
                    j=0
                    is_all_0 = 0
                    r1 = np.random.random((self.jizhan, self.mes))  # 该函数表示成size行 1列的浮点数，浮点数都是从0-1中随机。
                    r2 = np.random.random((self.jizhan, self.mes))
        return V,X

    def update(self):
        self.new_popu = np.zeros((self.pop, self.jizhan, self.mes), dtype=np.int)
        self.new_V = np.zeros((self.pop, self.jizhan, self.mes), dtype=np.int)
        a = np.array(self.fitness)
        ind = np.argsort(a)
        for i in range(int(self.pop)):
