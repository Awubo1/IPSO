...
class  GA():
    def __init__(self,length,dim,pop, max_iter):  #   长度  维度，群体数量，迭代次数
        ...
        self.tran_v=10*1024  ##MB/s=1024KB/s
    ...
    def com_fit1(self):
        ...
        if min_fitness < self.gbest_fitness:
            ...
            self.gbest = self.population[b[0]]  # 初始化整个整体的精英染色体
    ...
    def select(self):#选择
        ...
        ind=np.argsort(a)
        self.elite = []
        self.elite = self.population[ind[0]]
        self.elite_index = ind[0]
        # self.iter_bfit.append(self.fitness[ind[0]])
        for i in range(int((1/2)*self.pop),self.pop):
            ind[i]= ind[i-int((1/2)*self.pop)]
        # print(ind)
        for i in range(self.pop):
    ...
    def add_elite(self):
        a = int(random.randint(0, self.pop - 1))
        self.population[a] = self.elite

    def crossover(self):#交叉
        for i in range(0, self.pop - 1, 2):
            if random.random() < self.pc:
                for j in range(0, self.length):
                    # 随机选择交叉点
                    change_point = random.randint(0, self.dim - 1)
                    temp1 = np.zeros((1,self.length, self.dim), dtype=np.int)
                    temp2 = np.zeros((1,self.length, self.dim), dtype=np.int)
                    for k in range(change_point):
                        temp1[0][j][k] = self.population[i][j][k]
                        temp2[0][j][k] = self.population[i + 1][j][k]
                    for k in range(change_point, self.dim):
                        temp1[0][j][k] = self.population[i + 1][j][k]
                        temp2[0][j][k] = self.population[i][j][k]
                    self.population[i][j][k]= temp1[0][j][k]
                    self.population[i + 1][j][k]= temp2[0][j][k]
                    if self.population[i][j][k] > self.choice1[-1]:
                        self.population[i][j][k] = self.population[i][j][k] - self.choice1[-1]
                    elif self.population[i][j][k] < self.choice1[0]:
                        self.population[i][j][k] = -self.population[i][j][k]
    ...
    def mutation(self):  # 变异
        for i in range(self.pop):
            for j in range(self.length):
                for k in range(self.dim):
                    scope = []
                    for m in range(len(self.edge_index)):
                        scope.append(self.edge_index[m])
                    if random.random() < self.pm:
                        for n in range(len(self.edge_index)):
                            if self.new_popu[i][j][k] == self.edge_index[n]:
                                del scope[n]
                                break
                        self.new_popu[i][j][k] = random.choice(scope)
                        if self.population[i][j][k] > self.choice1[-1]:
                            self.population[i][j][k] = self.population[i][j][k] - self.choice1[-1]
                        elif self.population[i][j][k] <self.choice1[0]:
                            self.population[i][j][k] = -self.population[i][j][k]
